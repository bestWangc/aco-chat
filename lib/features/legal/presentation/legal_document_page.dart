...
const _userAgreementSections = [
  _LegalSection(
    '一、协议说明',
    '欢迎使用 Aco Chat。使用本应用、创建或导入钱包、注册账号或使用聊天、社区、直播等功能，即表示你已阅读、理解并同意本协议。未成年人应在监护人指导下使用本应用。',
  ),
  _LegalSection(
    '二、服务内容',
...
const _privacyPolicySections = [
  ...
  _LegalSection(
    '三、设备权限',
    '在你使用特定功能时，本应用可能请求相机权限以扫描二维码、麦克风权限以参与语音或直播、相册权限以选择图片，以及生物识别权限以保护钱包。你可以在系统设置中管理这些权限；拒绝非必要权限不会影响其他基础功能，但可能无法使用对应功能。',
  ),
  _LegalSection(
    '四、钱包数据与本地安全',
  ...
  _LegalSection(
    '五、信息共享与公开',
    '除非取得你的单独同意、为实现你主动选择的功能、履行法定义务、保护用户或公众安全，或为处理争议和维护合法权益，我们不会向第三方出售你的个人信息。你在公开社区、直播间或其他公开区域主动发布的内容，可能被其他用户查看和传播，请谨慎分享。',
  ),
  _LegalSection(
    '六、信息存储与保护',
